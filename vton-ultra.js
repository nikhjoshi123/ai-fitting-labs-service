(function() {
    ...
    // 1. PRECISION FILTER: ONLY PRODUCT PAGES
    const isStrictProductPage = () => {
        const path = window.location.pathname;
        const url = window.location.href;

        // BLOCK: Landing page, Cart, Account, and Search pages
        const blockedPaths = ['/cart', '/account', '/checkout', '/search', '/pages/contact'];
        if (path === "/" || path === "" || blockedPaths.some(p => path.includes(p))) return false;

        // ALLOW: If URL contains "product" or if there is an "Add to Cart" button present
        const hasProductInUrl = url.toLowerCase().includes('/products/') || url.toLowerCase().includes('/item/');
        const hasAddToCart = !!document.querySelector('button[name="add"], .add-to-cart, #AddToCart, [data-testid="add-to-cart"]');
        
        return hasProductInUrl || hasAddToCart;
    };
    ...
    const injectStyles = () => {
        ...
        s.innerHTML = `
            ...
            #v-done { width: 100%; padding: 20px; background: #000; color: #fff; border-radius: 15px; border: none; font-weight: bold; cursor: pointer; font-size: 16px; margin-top: 30px; transition: 0.3s; }
            #v-done:hover { opacity: 0.8; }
    ...
    // 3. PERSISTENT BUTTON CREATOR
    const createButton = () => {
        const isProduct = isStrictProductPage();
        ...
        btn.style.cssText = "position:fixed; bottom:30px; right:30px; z-index:2147483647; padding:18px 36px; color:#fff; border-radius:50px; font-weight:bold; border:none; cursor:pointer; box-shadow:0 15px 35px rgba(0,0,0,0.4); background: linear-gradient(135deg, #000, #444); display:block !important; font-size: 15px; transition: transform 0.2s;";
        ...
        input.onchange = (e) => {
            ...
            reader.onload = async (re) => {
                try {
                    ...
                    else alert("Connection error. Please check Vercel logs.");
                }
            };
    ...
    // START & MAINTAIN
    injectStyles();
    setInterval(createButton, 1500); // Check for page changes every 1.5s
})();
