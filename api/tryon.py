...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if "check=" in self.path:
            ...
            try:
                m_resp = requests.get(f"{BASE_SHEETY}/sheet1", timeout=8)
                rows = m_resp.json().get("sheet1", [])
                client_data = next((r for r in rows if str(r.get("clientKey")).strip() == client_key), None)
                if client_data and int(client_data.get("usage", 0)) >= int(client_data.get("limit", 0)):
    ...
    def do_POST(self):
        try:
            ...
            # Security: If paused, don't even call the AI
            ...
            # IP Check
            l_resp = requests.get(f"{BASE_SHEETY}/sheet2", timeout=8)
            user_log = next((l for l in l_resp.json().get("sheet2", []) if str(l.get("ip")).strip() == user_ip), None)
            p_limit = int(client_data.get("userTrialLimit", 5))
            if (user_log.get("count", 0) if user_log else 0) >= p_limit:
                self._set_headers(403)
                self.wfile.write(json.dumps({"error": f"IP Limit Reached"}).encode())
                return
            ...
            if "id" in ai_resp:
                requests.put(f"{BASE_SHEETY}/sheet1/{client_data['id']}", json={"sheet1": {"usage": int(client_data['usage']) + 1}})
                if user_log:
                    requests.put(f"{BASE_SHEETY}/sheet2/{user_log['id']}", json={"sheet2": {"count": int(user_log['count']) + 1}})
                else:
                    requests.post(f"{BASE_SHEETY}/sheet2", json={"sheet2": {"ip": user_ip, "count": 1}})
                ai_resp["is_premium"] = str(client_data.get("isPremium", "FALSE")).upper() == "TRUE"
                self._set_headers(200)
                self.wfile.write(json.dumps(ai_resp).encode())
            else:
                self._set_headers(400)
                self.wfile.write(json.dumps({"error": "AI Error"}).encode())
